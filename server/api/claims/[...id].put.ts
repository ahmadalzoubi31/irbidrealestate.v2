import { claim, claimCollection, claimDetail } from "@prisma/client";
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const deleteCollectionOperations = idsCollectionToDelete.map((id) =>
        tx.claimCollection.delete({ where: { id: id } })
      );
      // Perform deletions
      const deleteDetailOperations = idsDetailToDelete.map((id) =>
        tx.claimDetail.delete({ where: { id: id } })
      );

      // Handle updates and creations
      const upsertCollectionOperations = claimCollections.map(
        (c: claimCollection) =>
          c.id
