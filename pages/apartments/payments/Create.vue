...
const computedApartments = computed(() =>
  availableApartments.value
    ?.filter(
      (a) =>
        a.buildingId == selectedBuildingId.value &&
        (a.rentStatus === 2 || a.rentStatus === 3)
    )
...
const fillRentAmount = computed(
  () =>
    availableApartments.value?.find((a) => a.id == state.apartmentId)
      ?.rentAmount
);
const fillRentDate = computed(
  () =>
    availableApartments.value?.find((a) => a.id == state.apartmentId)?.rentDate
);
const fillCommissionAmount = computed(
  () =>
    availableApartments.value?.find((a) => a.id == state.apartmentId)
      ?.commissionAmount
);
// @ts-ignore
const fillMaintenanceDiscount = computed(
  () =>
    availableApartments.value?.find((a) => a.id == state.apartmentId)?.building
      .maintenanceAmount
);
// @ts-ignore
const fillServices = computed(
  () =>
    availableApartments.value?.find((a) => a.id == state.apartmentId)?.building
      .serviceAmount
);
      ...
      <div class="grid grid-cols-6 gap-x-6 gap-y-4">
        ...
        <div class="col-span-6 sm:col-span-2">
          ...
          <UInput
            ...
            :disabled="true"
            :model-value="
              fillRentDate ? format(new Date(fillRentDate), 'dd/MM/yyyy') : ''
            "
          />
        </div>
        <!-- rentAmount -->
        ...
        <!-- nextRentDate -->
        <div class="col-span-6 sm:col-span-2">
          <label for="nextRentDate" class="text-primary-600">
          ...
          <UInput
            ...
            :model-value="
              calculateNextRentDate
                ? format(calculateNextRentDate, 'dd/MM/yyyy')
                : ''
            "
          />
        </div>
