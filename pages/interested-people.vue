...
// Filtering
const filteredRows = useFilteredRows<InterestedPeople>(interestedPeoples, q, ["createdAt", "updatedAt"]);
    ...
    <div class="parentWrapper" v-if="useRoute().name === 'interested-people'">
      ...
      <div class="flex my-3 justify-between">
        <div id="buttonWrapper">
          <UButton icon="i-heroicons-plus-circle-20-solid" label="اضافة شخص " :to="'/interested-people/create'" />
          <UButton icon="i-heroicons-minus-circle-20-solid" label="حذف شخص " :disabled="selected.length === 0" @click="deleteSelectedRecord" />
        </div>
      ...
      <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-[0.25rem] mb-2">
        <UTable :rows="filteredRows" :columns="selectedColumns" v-model="selected" :single-select="true" @select="select" :loading="isLoading">
